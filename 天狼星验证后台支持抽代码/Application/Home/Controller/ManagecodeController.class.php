...
class ManagecodeController extends BaseController {
    ...
    private function init() {
        ...
        if (!isset($_GET['search'])) {
            // 默认搜索全部
            parent::showTableAndPage('regcode',null);
            return;
        }
        $this->searchHandle();
    }
    ...
    private function exportTxt($op) {
        $today = date('Ymd');
        $filename='天狼星_' . $today . '.txt';//要导出的文件的文件名需要加上文件后缀
        ...
        $chars = $chars . '          注册码                           软件                时长                生成时间                             到期时间                          授权次数                 备注' . "\r\n";
        foreach ($ret as $json) {
            if ($op == 'export1') { //注册码和推荐码
                $chars = $chars . '注册码:' . $json['code'] . ' 推荐码:' . ($json['id'] + 10000) . "\r\n";
            } else { // 注册码
                $chars = $chars . $json['code'].'               '.$json['software_name'].'        '.$json['all_minutes'].'            '.$json['produce_time'].'            '.$json['expire_time'].'           '.$json['use_count'].'               '.$json['mark']."\r\n";
            }
        }
